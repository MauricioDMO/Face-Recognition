...
def init():
    ...
    cursor.executemany(
        """INSERT INTO classrooms (classroom, building_id) values (?, ?)""", classrooms
    )
    CONNECTION.commit()
    ...
    hours = [("06:20", "08:00"), ("08:10", "09:50"), ("10:00", "11:40")]
    # ? insert hours into the table hours.
    cursor.executemany(
        """INSERT INTO hours (start_time, end_time) values (?, ?)""", hours
    )
    CONNECTION.commit()
    ...
    cursor.executemany(
        """INSERT INTO schedules (day_id, hour_id) values (?, ?)""", schedules
    )
    CONNECTION.commit()
